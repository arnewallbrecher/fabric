...
procedure TForm45.ButtonSabbrechenClick(Sender: TObject);
begin
  PanelSpeichern.Visible := false;
end;
...
begin
  ...
  begin
    ...
    DecimalSeparator := '.';
    XS := Inttostr(round((Xnull+Seite/2) * 400));
    YS := IntToStr(round((Ynull+Seite) * 400));
...
procedure TForm45.ButtonPlotEndeClick(Sender: TObject);
begin
  GroupBoxPlotten.Visible := false;
  PanelCWert.Visible      := true;
end;
...
procedure TForm45.TextschreibenClick(Sender: TObject);
begin
...
procedure TForm45.SpeedTextClick(Sender: TObject);
begin
  TextschreibenClick(Sender);
end;

procedure TForm45.MaskEditTextKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = chr(13) then
  begin
...
procedure TForm45.Image1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if BText then
  begin
...
procedure TForm45.TrueckClick(Sender: TObject);
begin
...
procedure TForm45.SpeedUnTextClick(Sender: TObject);
begin
  TrueckClick(Sender);
end;
