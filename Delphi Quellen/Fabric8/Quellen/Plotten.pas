...
Procedure DemoText (Canvas: TCanvas; X,Y: word);
...
procedure RotationT(Canvas: TCanvas; Az, F, Da, Df: real;
...
procedure AchsenPlot (Canvas: TCanvas; Aq:array of word;
                      Fq:array of shortint; Def: string);
...
procedure RotEllipseT(Canvas: TCanvas; Lang, Kurz: real; Phi: integer; Breite, Hoehe: word);
procedure Pplotten (Canvas:TCanvas; Xp,Yp : word; n: byte);
procedure Konturen (Canvas1: TCanvas; FuellenS: string;
                    Fortschritt: TProgressbar);
Procedure Symbole (Canvas: TCanvas; Xp, Yp: word; Dsymbol: string);
procedure AziFall (X, Y: word; var Azimut, Fallen:word);
...
begin
  ...
  begin
    ...
    Font.Color  := clGray;
    if HintFarbe = clGray then
      Font.Color := clBlack;
    Brush.Color := HintFarbe;
    Font.Size   := 20;
    Textout(X, Y, 'Demo-Version');
  end;
end;
...
begin
  ...
  begin
    ...
    repeat
      ...
      if Y1 <> 0 then Ta := arctan(Y2 / Y1) else Ta := 10000;
      if Y1 <= 0 then Ta := Ta + Pi;
      if Y3 > 0.99999 then Fi := 0 else Fi := Pi/2-arctan(Y3/sqrt(1-sqr(Y3)));
      Aa := Ta;
      Ff := abs(Pi / 2 - Fi);
      ...
      begin
        if sqr(Y - Yl) + sqr(X - Xl) < 0.999 then
        begin
          Pen.Color := Kegel;
          lineto(Xp,Yp);
        end;
...
begin
  ...
  begin
    Pen.Color := Kfarbe;
    moveto(Xp,Yp);
    Rotation(Canvas, A, F, Da, Df, 0, 360, 1, Kfarbe);
//    lineto(Xp,Yp);
  end;
end;
...
for i := 1 to 3 do
  ...
  if ARot2 <= 180 then Schritt := 1 else Schritt := -1;
  with Canvas do
  begin
    for i := 1 to 3 do
    begin
      ...
      moveto(Xp,Yp);
      RotationT (Canvas, A[i], F[i], Da[i], Df[i], 0, 90,
                Schritt, EigenFarbe);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Def = 'Komp' then
      begin
         if k = 1 then TextOut (Xp-5, Yp-13, 'Y');  //chr(87+k));
         if k = 2 then TextOut (Xp-5, Yp-13, 'X');
         if k = 3 then TextOut (Xp-5, Yp-13, 'Z');
      end;
      if Def = 'Dehn' then
      begin
        if k = 1 then TextOut (Xp-5, Yp-13, 'Y');    //chr(87 + k));
        if k = 2 then TextOut (Xp-5, Yp-13, 'Z');
        if k = 3 then TextOut (Xp-5, Yp-13, 'X');
      end;
    end;
    Font.Style := [];
  end;
...
//    for i := 0 to n do
//    begin
//      moveto (Xnull + round(Tick*i),Ynull);
//      lineto (Xnull + round(Tick*i), Ynull+ 10);
//    end;
//    for i := 0 to n do
//    begin
//      moveto (Xnull, Ynull - round(Tick*i));
//      lineto (Xnull-10,Ynull - round(Tick*i));
//    end;
    Brush.Color := HintFarbe;
    ...
    begin
      ...
      begin
        moveto (Xnull + round(Tick*i),Ynull);
        lineto (Xnull + round(Tick*i), Ynull+ 10);
        Textout (Xnull+round(Tick*i)+3, Ynull+4,Inttostr(i+1));
      end;
      for i := 0 to n do
      begin
        moveto (Xnull, Ynull - round(Tick*i));
        lineto (Xnull-10,Ynull - round(Tick*i));
        Textout (Xnull-20, Ynull-round(Tick*i)-12,Inttostr(i+1));
      end;
    end;
    ...
    begin
      ...
      begin
        moveto (Xnull + round(2*Tick*i),Ynull);
        lineto (Xnull + round(2*Tick*i), Ynull+ 10);
        Textout (Xnull+round(2*Tick*i)+3, Ynull+4,Inttostr(2*i+1));
      end;
      for i := 0 to n div 2 do
      begin
        moveto (Xnull, Ynull - round(2*Tick*i));
        lineto (Xnull-10,Ynull - round(2*Tick*i));
        Textout (Xnull-20, Ynull-round(2*Tick*i)-12,Inttostr(2*i+1));
      end;
    end;
    if (zoom > 40) and (zoom <=80) then
    begin
      font.Size := 8;
      for i := 0 to n div 4 do
      begin
        moveto (Xnull + round(4*Tick*i),Ynull);
        lineto (Xnull + round(4*Tick*i), Ynull+ 10);
        Textout (Xnull+round(4*Tick*i)+3, Ynull+4,Inttostr(4*i+1));
      end;
      for i := 0 to n div 4 do
      begin
        moveto (Xnull, Ynull - round(4*Tick*i));
        lineto (Xnull-10,Ynull - round(4*Tick*i));
        Textout (Xnull-20, Ynull-round(4*Tick*i)-12,Inttostr(4*i+1));
      end;
    end;
    if (zoom > 80) and (zoom <=120) then
    begin
      font.Size := 8;
      for i := 0 to n div 8 do
      begin
        moveto (Xnull + round(8*Tick*i),Ynull);
        lineto (Xnull + round(8*Tick*i), Ynull+ 10);
        Textout (Xnull+round(8*Tick*i)+3, Ynull+4,Inttostr(8*i+1));
      end;
      for i := 0 to n div 8 do
      begin
        moveto (Xnull, Ynull - round(8*Tick*i));
        lineto (Xnull-10,Ynull - round(8*Tick*i));
        Textout (Xnull-20, Ynull-round(8*Tick*i)-12,Inttostr(8*i+1));
      end;
    end;
    if (zoom > 120) and (zoom <=200) then
    begin
      font.Size := 6;
      for i := 0 to n div 20 do
      begin
        moveto (Xnull + round(20*Tick*i),Ynull);
        lineto (Xnull + round(20*Tick*i), Ynull+ 10);
        Textout (Xnull+round(20*Tick*i)+3, Ynull+4,Inttostr(20*i+1));
      end;
      for i := 0 to n div 12 do
      begin
        moveto (Xnull, Ynull - round(20*Tick*i));
        lineto (Xnull-10,Ynull - round(20*Tick*i));
        Textout (Xnull-20, Ynull-round(20*Tick*i)-12,Inttostr(20*i+1));
      end;
    end;
    if (zoom > 200) then
    begin
      font.Size := 6;
      for i := 0 to n div 40 do
      begin
        moveto (Xnull + round(40*Tick*i),Ynull);
        lineto (Xnull + round(40*Tick*i), Ynull+ 10);
        Textout (Xnull+round(40*Tick*i)+3, Ynull+4,Inttostr(40*i+1));
      end;
      for i := 0 to n div 12 do
      begin
        moveto (Xnull, Ynull - round(40*Tick*i));
        lineto (Xnull-10,Ynull - round(40*Tick*i));
        Textout (Xnull-20, Ynull-round(40*Tick*i)-12,Inttostr(40*i+1));
      end;
    end;
    Font.Size := 16;
    ...
    begin
      ...
      begin
        ...
        begin
          with Canvas do
          begin
            if Zaehl >= 1 then PixelS[x,y] := Isolinie[1];
            if Zaehl >= 2 then PixelS[x,y] := Isolinie[2];
            if Zaehl >= 4 then PixelS[x,y] := Isolinie[3];
            if Zaehl >= 8 then PixelS[x,y] := Isolinie[4];
            if Zaehl >= 16 then PixelS[x,y] := Isolinie[5];
          end;
        end;
...
begin
  ...
  begin
    ...
    for i := 1 to 360 do
    begin
      Grenze := sqrt(sqr(Xp-xr)+sqr(yp-yr));
...
begin
  ...
  with Canvas do
  begin
    if LeftS(DSymbol,4) = 'Open' then Brush.Color := DiagrFarbe;
    if RightS(DSymbol,6) = 'Circle' then Ellipse (Xp-n,Yp-n,Xp+n,Yp+n);
    if RightS(DSymbol,6) = 'Square' then Rectangle (Xp-n,Yp-n,Xp+n,Yp+n);
    if RightS(Dsymbol,8) = 'Triangle' then Polygon([Point(Xp,Yp-n),
                                                Point(Xp-n,Yp+n),
                                                Point(Xp+n,Yp+n)]);
    if RightS(DSymbol,7) = 'Diamond' then Polygon ([Point(Xp,Yp-n),
                                                 Point(Xp+n,Yp),
                                                 Point(Xp,Yp+n),
                                                 Point(Xp-n,Yp)]);
  end;
...
begin
  ...
  begin
    ...
    Azim   := round(AziExt);
    if Azim < 0 then Azim := Azim + 180;
    if (Azim = 0) and (KoordY < 0) then Azim := 180;
    if (Azim = 0) and (KoordY > 0) then Azim := 0;
    if (Azim <> 0) and (Azim <> 180) then
      if KoordX < 0 then Azim := Azim + 180;
    if Azim >=360 then Azim := Azim - 360;
