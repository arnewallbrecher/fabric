...
begin
  ...
  begin
    ...
    begin
      if ListboxAusgDateien.Selected[i] then
      begin
        LoeschS := ListBoxAusgDateien.Items.Strings[i];
...
begin
  ...
  begin
    ...
    if not Talbot[i] then ListBoxKompression.Items.Add (ZeileS);
    if Talbot[i] then ListBoxDehnung.Items.Add (ZeileS);
  end;
  ButtonAuswahl.Enabled := false;
end;
...
begin
  ...
  begin
    ...
    HelpRouter1.HelpJump(DateiS,'html\Deutsch\62Talbot-dis.htm');
  end;
  if SpracheS = 'ENGLISH' then
  begin
    DateiS := DirectS+'\'+'Fabric-Help-Html';
    HelpRouter1.HelpJump(DateiS,'html\English\62Talbot-dis.htm');
  end;
  HelpRouter1.free;
end;
...
procedure TForm62.loeschenClick(Sender: TObject);
begin
  Loesch := true;
  showmessage(Tx(1719)+chr(10)+chr(13)+Tx(1720));
end;

procedure TForm62.SpeedLoeschenClick(Sender: TObject);
begin
