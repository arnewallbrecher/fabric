...
type
  TForm100 = class(TForm)
    ...
    PanelMspeichern: TPanel;
    ListBoxMSpeichern: TListBox;
    ButtonMabbrechen: TButton;
    PanelHspeichern: TPanel;
    ListBoxHspeichern: TListBox;
    ButtonHabbrechen: TButton;
    Text: TMenuItem;
    ...
    LabelText: TLabel;
    LabelReturn: TLabel;
    MaskEditMText: TMaskEdit;
    PanelMPlazieren: TPanel;
    LabelPlazieren: TLabel;
    TextMohr: TMenuItem;
    TextHisto: TMenuItem;
    RueckMohr: TMenuItem;
    RueckHisto: TMenuItem;
    PanelHText: TPanel;
    Label1: TLabel;
    Label2: TLabel;
    MaskEditHText: TMaskEdit;
    PanelHPlazieren: TPanel;
    Label3: TLabel;
    SpeedTextH: TSpeedButton;
    SpeedUnTextH: TSpeedButton;
    SpeedUnTextM: TSpeedButton;
    SpeedTextM: TSpeedButton;
   procedure ShowHint(Sender: TObject);
    ...
    procedure ButtonHabbrechenClick(Sender: TObject);
    procedure ButtonMabbrechenClick(Sender: TObject);
    procedure MaskEditHTextKeyPress(Sender: TObject; var Key: Char);
    procedure RueckMohrClick(Sender: TObject);
    procedure RueckHistoClick(Sender: TObject);
    procedure TextMohrClick(Sender: TObject);
    procedure TextHistoClick(Sender: TObject);
    procedure ImageHistoMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure MaskEditMTextKeyPress(Sender: TObject; var Key: Char);
    procedure SpeedTextHClick(Sender: TObject);
    procedure SpeedTextMClick(Sender: TObject);
    procedure SpeedUnTextMClick(Sender: TObject);
    procedure SpeedUnTextHClick(Sender: TObject);
  end;
...
var
  ...
  MBText          : boolean;
  HBText          : boolean;
  MTextBitmap     : array[1..10] of TBitmap;
  HTextBitmap     : array[1..10] of TBitmap;
  Mtexti          : byte;
  Htexti          : byte;
  TMohr           : boolean;
...
begin
  ...
  SpeedTextM.Hint          := Hilf(731);
  SpeedTextH.Hint          := Hilf(732);
  SpeedUnTextM.Hint        := Hilf(733);
  SpeedUnTextH.Hint        := Hilf(734);
  ...
  ButtonMabbrechen.Caption := Tx(1682);
  ...
  PanelMspeichern.Visible := false;
  for i := 1 to 7 do
    ListBoxHSpeichern.Items.Add (Tx(1611+i));
  PanelHspeichern.Visible := false;
  ...
  PanelMText.Visible := false;
  PanelHText.Visible := false;
  MBText             := false;
  HBText             := false;
  Mtexti             := 0;
  HTexti             := 0;
  RueckMohr.Enabled  := false;
  RueckHisto.Enabled := false;
  TMohr              := false;
end;
...
begin
  ...
  begin
    ...
    begin
      inc (Mtexti);
      MTextBitmap[Mtexti] := Tbitmap.Create;
      MTextBitmap[Mtexti].Assign(ImageMohr.Picture);
      with ImageMohr.Canvas do
      begin
      ...
      ImageMohr.Canvas.TextOut(x,y,MTeS);
      PanelMText.Visible := false;
      MBText := false;
      RueckMohr.Enabled    := true;
      SpeedUnTextM.Enabled := true;
    end;
  end
  else
  begin
...
procedure TForm100.ButtonMabbrechenClick(Sender: TObject);
begin
  PanelMspeichern.Visible := false;
end;
...
procedure TForm100.ButtonHabbrechenClick(Sender: TObject);
begin
  PanelHspeichern.Visible := false;
end;
...
begin
  ...
  if Druck = 2 then
    Hoehe := round(PLaenge*ImageHisto.Height/ImageHisto.Width);;
  with Fenster do
  begin
    Left   := LRand;
    Top    := ORand;
  ...
  try
    ...
    if Druck = 2 then
      Bitmap.Assign(ImageHisto.picture.graphic);
    with Printer do
    begin
...
procedure TForm100.TextMohrClick(Sender: TObject);
begin
  PanelMText.Visible := true;
  MaskEditMText.SetFocus;
  MBText := true;
  TMohr  := true;
  MaskEditMText.Clear;
  PanelMPlazieren.Visible := false;
end;

procedure TForm100.SpeedTextMClick(Sender: TObject);
begin
  TextMohrClick(Sender);
end;

procedure TForm100.TextHistoClick(Sender: TObject);
begin
  PanelHText.Visible := true;
  MaskEditHText.SetFocus;
  HBText := true;
  MaskEditHText.Clear;
  PanelHPlazieren.Visible := false;
end;

procedure TForm100.SpeedTextHClick(Sender: TObject);
begin
  TextHistoClick(Sender);
end;

procedure TForm100.MaskEditMTextKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = chr(13) then
  begin
    MTeS := MaskEditMText.Text;
    PanelMPlazieren.Visible := true;
  end;
end;

procedure TForm100.MaskEditHTextKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = chr(13) then
  begin
    HTeS := MaskEditHText.Text;
    PanelHPlazieren.Visible := true;
  end;
end;


procedure TForm100.ImageHistoMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if HBText then
  begin
    inc (Htexti);
    HTextBitmap[Htexti] := Tbitmap.Create;
    HTextBitmap[Htexti].Assign(ImageHisto.Picture);
    with ImageHisto.Canvas do
    begin
      Brush.Color := DiagrFarbe;
      Font.Name   := DSchrift;
    ...
    ImageHisto.Canvas.TextOut(x,y,HTeS);
    PanelHText.Visible := false;
    HBText := false;
    RueckHisto.Enabled   := true;
    SpeedUnTextH.Enabled := true;
  end;
end;

procedure TForm100.RueckMohrClick(Sender: TObject);
begin
  ImageMohr.Canvas.Draw (0,0,MTextBitmap[Mtexti]);
  if Mtexti >= 1 then
  begin
    MTextBitmap[Mtexti].free;
    Dec(Mtexti);
  end;
  if Mtexti = 0 then
  begin
    RueckMohr.Enabled    := false;
    SpeedUnTextM.Enabled := false;
  end;
  TMohr := false;
end;

procedure TForm100.SpeedUnTextMClick(Sender: TObject);
begin
  RueckMohrClick(Sender);
end;

procedure TForm100.RueckHistoClick(Sender: TObject);
begin
  ImageHisto.Canvas.Draw (0,0,HTextBitmap[Htexti]);
  if Htexti >= 1 then
  begin
    TextBitmap[Htexti].free;
    Dec(Htexti);
  end;
  if Htexti = 0 then
  begin
    RueckHisto.Enabled   := false;
    SpeedUnTextH.Enabled := false;
  end;
end;

procedure TForm100.SpeedUnTextHClick(Sender: TObject);
begin
  RueckHistoClick(Sender);
end;
