...
begin
  try
    ...
    begin
      Lauf := chr(i+64);
      if FindFirst(Lauf + ':\FabricIni',faAnyFile,Pfad) = 0 then
      begin
        Finde := 0;
        FabricS := Lauf + ':\' + PFad.Name;
        FindClose(Pfad);
        exit;
      end
      else continue;
  ...
  showmessage('Could not find FabricIni!'+Chr(13)+'  '+chr(13)
               +'Connect a memory device to the computer'+chr(13)
               +'that contains FabricIni.');
end;
