...
begin
  ...
  begin
    ...
    Rectangle (35, 30, Breite+35, Hoehe+30);
    Brush.Color := HintFarbe;
    for i := 1 to 17 do
    begin
      Textout(8, round(13 + 0.1*i+Hoehe*i/18),IntTostr(90-i*10)+'°');
      moveto (35, round(30 + Hoehe*i/18));
      lineto (20, round(30 + Hoehe*i/18));
    end;
    MaxRf := trunc(RfMax) + 2;
    MinRf := trunc(RfMin);
    ...
    begin
      Xr := 35+round(ln(i*e)*Dist-Dist);
      Yr := Hoehe+29;
      moveto (Xr, Yr);
      lineto (Xr, Yr-10);
      if i mod 2 = 0 then
        Textout(Xr-4, Yr+2, IntToStr(i));
      Yr := 30;
      moveto (Xr, Yr);
      lineto (Xr, Yr+10);
      if i mod 2 = 0 then
        Textout(Xr-4, Yr-18, IntToStr(i));
    end;
    Font.Name   := 'Symbol';
    Font.Size   := 16;
    Font.Color  := KoordinFarbe;
    ...
    begin
      Xr := 35 + round(ln(Rf[i]*e)* Dist-Dist);
      Yr := round(30+Hoehe/2-RPhi[i]*Hoehe/180);
...
begin
  ...
  Hmittel := n/Nenner;
  Xmitte  := 35+round(ln(Hmittel*e)*Dist-Dist);
end;
...
begin
  ...
  Ymitte := 30 + round(Hoehe/2-Vmittel*Hoehe/180);
  Vm     := round(Vmittel);
