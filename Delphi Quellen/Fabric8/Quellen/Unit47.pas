...
procedure TForm47.ButtonSabbrechenClick(Sender: TObject);
begin
  PanelSpeichern.Visible := false;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if GeraetS = 'DXF' then
          Dxftext (Xnull,Ynull,Seite/3,DiagNameS,'0.00');
      end;
    end;
    XS := Inttostr(round((Xnull) * 400));
    YS := IntToStr(round((Ynull+Seite/10) * 400));
    WS := FloatToStr(Seite/45);
    HS := FloatToStr(Seite/30);
...
procedure TForm47.ButtonPlotEndeClick(Sender: TObject);
begin
  GroupBoxPlotten.Visible := false;
  PanelCWert.Visible      := true;
end;
...
procedure TForm47.TextschreibenClick(Sender: TObject);
begin
...
procedure TForm47.SpeedTextClick(Sender: TObject);
begin
  TextschreibenClick(Sender);
end;

procedure TForm47.MaskEditTextKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = chr(13) then
  begin
...
procedure TForm47.Image1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
 if BText then
  begin
...
procedure TForm47.TrueckClick(Sender: TObject);
begin
...
procedure TForm47.SpeedUnTextClick(Sender: TObject);
begin
  TrueckClick(Sender);
end;
