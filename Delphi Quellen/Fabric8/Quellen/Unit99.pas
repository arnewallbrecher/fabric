...
type
  TForm99 = class(TForm)
    ...
    procedure ImageFluktMouseUp(Sender: TObject; Button: TMouseButton;
...
procedure TForm99.ButtonIabbrechenClick(Sender: TObject);
begin
  PanelIspeichern.Visible := false;
end;
...
begin
  ...
  begin
    ...
    YS := IntToStr(round((YNull-YNull/5)*400));
    WS := FloatToStr(Seiteh/20);
    HS := FloatToStr(Seiteh/15);
    ...
    if GeraetS = 'DXF' then
      Dxftext (XNull, Ynull-Seiteh/10,Seiteh/3,DiagNameS,'0.00');
    Teils   := 10;
    XanNeu  := XNull + Seiteh;
    YanNeu  := YNull + Seitev;
    ...
    XS := FloatToStr((XNull+Seiteh/3)*400);
    YS := FloatToStr((YNull+0.94*Seitev)*400);
    if GeraetS = 'DXF' then
    begin
      YS := FloatToStr((YNull+0.9*Seitev)*400);
      HpRadius := Seiteh/2;
    end;
    WS := FloatToStr(Seiteh/30);
    HS := FloatToStr(Seiteh/20);
    Hpende (XS, Ys, WS, HS);
  end;
  if GeraetS = 'Plotter' then
  begin
...
procedure TForm99.ButtonPlotEndeClick(Sender: TObject);
begin
  GroupBoxPlotten.Visible := false;
  PanelEingabe.Visible    := false;
end;
...
procedure TForm99.ButtonMabbrechenClick(Sender: TObject);
begin
  PanelMspeichern.Visible := false;
end;
...
begin
  ...
  if Druck = 1 then
    Hoehe   := round(PLaenge*ImageFlukt.Height/ImageFlukt.Width);;
  if Druck = 2 then
  ...
  try
    if Druck = 1 then
      Bitmap.Assign(ImageFlukt.picture.graphic);
    if Druck = 2 then
...
procedure TForm99.TextschreibenClick(Sender: TObject);
begin
...
procedure TForm99.SpeedTextClick(Sender: TObject);
begin
  TextschreibenClick(Sender);
end;

procedure TForm99.MaskEditTextKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = chr(13) then
  begin
...
procedure TForm99.ImageFluktMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if BText then
  begin
    ...
    TextBitmap[texti].Assign(ImageFlukt.Picture);
    with ImageFlukt.Canvas do
    begin
      Brush.Color := DiagrFarbe;
      Font.Name   := DSchrift;
      ...
      Font.Color  := DFarbe;
      Font.Style  := DStyle;
      ImageFlukt.Canvas.TextOut(x,y,TeS);
      PanelText.Visible := false;
      BText := false;
    end;
    Trueck.Enabled      := true;
...
procedure TForm99.TrueckClick(Sender: TObject);
begin
  ImageFlukt.Canvas.Draw (0,0,TextBitmap[texti]);
  if texti >= 1 then
  begin
...
procedure TForm99.SpeedUnTextClick(Sender: TObject);
begin
  TrueckClick(Sender);
end;
