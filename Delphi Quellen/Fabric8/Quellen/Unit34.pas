...
type
  TForm34 = class(TForm)
    ...
    PanelFSpeichern: TPanel;
    ListBoxFSpeichern: TListBox;
    ButtonFabbrechen: TButton;
    ...
    procedure ButtonFabbrechenClick(Sender: TObject);
    procedure MaskEditASigma1KeyPress(Sender: TObject; var Key: Char);
    procedure MaskEditFSigma1KeyPress(Sender: TObject; var Key: Char);
    procedure MaskEditASigma3KeyPress(Sender: TObject; var Key: Char);
    procedure MaskEditFSigma3KeyPress(Sender: TObject; var Key: Char);
...
begin
  ...
  ButtonFabbrechen.Caption         := Tx(1682);
...
begin
  ...
  for i := 1 to ZahlGlob do  {Umwandlung der Normalen in Flächen}
  begin
    AziFeld[i] := (180 + AziFeld[i]) mod 360;
    FalFeld[i] := 90 - FalFeld[i];
  end;
...
begin
  ...
  ListBoxMspeichern.Clear;
  ListBoxFSpeichern.Clear;
  ListBoxDateien.Clear;
  ListBoxtheoretisch.Clear;
  CheckBoxOK.State := cbUnchecked;
...
procedure TForm34.MaskEditASigma1KeyPress(Sender: TObject; var Key: Char);
begin
  if key = chr(13) then
  begin
    ASigma1S := MaskEditASigma1.Text;
    LeerLoeschen (ASigma1S);
    if ASigma1S = '' then exit
    else
    begin
      MaskEditFSigma1.SetFocus;
    end;
  end;
  CheckBoxOk.State := cbUnchecked;
end;

procedure TForm34.MaskEditFSigma1KeyPress(Sender: TObject; var Key: Char);
begin
  if key = chr(13) then
  begin
    FSigma1S := MaskEditFSigma1.Text;
    LeerLoeschen (FSigma1S);
    if FSigma1S = '' then exit
    else
    begin
      MaskEditASigma3.SetFocus;
    end;
  end;
  CheckBoxOk.State := cbUnchecked;
end;

procedure TForm34.MaskEditASigma3KeyPress(Sender: TObject; var Key: Char);
begin
  if key = chr(13) then
  begin
    ASigma3S := MaskEditASigma3.Text;
    LeerLoeschen (ASigma3S);
    if ASigma3S = '' then exit
    else
    begin
      MaskEditFSigma3.SetFocus;
    end;
  end;
  CheckBoxOk.State := cbUnchecked;
end;

procedure TForm34.MaskEditFSigma3KeyPress(Sender: TObject; var Key: Char);
begin
  if key = chr(13) then
  begin
    FSigma3S := MaskEditFSigma3.Text;
    LeerLoeschen (FSigma3S);
    if FSigma3S = '' then exit;
  end;
  CheckBoxOk.State := cbUnchecked;
end;
...
procedure TForm34.ButtonFabbrechenClick(Sender: TObject);
begin
  PanelFspeichern.Visible := false;
end;
...
procedure TForm34.ButtonMabbrechenClick(Sender: TObject);
begin
  PanelMspeichern.Visible := false;
end;
...
begin
  ...
  begin
    ...
    XS := Inttostr(round((XNull+0.24*Seite) * 400));
    YS := IntToStr(round((YNull+0.3*Seite) * 400));
    DecimalSeparator := '.';
    WS := FloatToStr(Seite/40);
    HS := FloatToStr(Seite/25);
...
procedure TForm34.ButtonPlotEndeClick(Sender: TObject);
begin
  GroupBoxPlotten.Visible := false;
end;
...
begin
  ...
  if Druck = 2 then
    Hoehe   := round(PLaenge*Image2.Height/Image2.Width);;
  with Fenster do
  begin
    Left   := LRand;
    Top    := ORand;
...
procedure TForm34.TextschreibenClick(Sender: TObject);
begin
...
procedure TForm34.SpeedTextClick(Sender: TObject);
begin
  TextschreibenClick(Sender);
end;

procedure TForm34.MaskEditTextKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = chr(13) then
  begin
...
procedure TForm34.Image1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if BText then
  begin
...
procedure TForm34.TrueckClick(Sender: TObject);
begin
...
procedure TForm34.SpeedUnTextClick(Sender: TObject);
begin
  TrueckClick(Sender);
end;
