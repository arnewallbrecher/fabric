...
uses Printers, Globvar, Texte, Dialogs, SysUtils, Basic,
     Graphics, Standard, Windows, Einausga, StdCtrls;

procedure Titel (TitelS: array of string; Fs:byte;
                 Az1, Az2, Fa1, Fa2, AzErg, FaErg: array of word;
                 Anzahl:word; DateiZahl:byte; NS: array of shortstring);
...
procedure Titel8 (KopfS:string;DateiZahl:byte; TitelS:array of string;
                  DatAz, DatFall:array of word; Zahl:word);
procedure TitelWel (DateiZahl:byte; TitelS:array of string;
                    Xschn, Yschn:array of word; Zahl:word);
procedure TitelFlinn(DateiZahl:byte; TitelS:array of string;
                     DLang, DMittel, DKurz:array of word; Zahl:word);
procedure TitelKarte (DateiZahl:byte; TitelS:array of string);
procedure Windatei (KopfS:string;DateiZahl:byte; SDatei: string;
                  TitelS:array of string;
                  DatAz, DatFall: array of word; Zahl:word);
procedure WinDatWel (DateiZahl:byte; SDatei: string; TitelS:array of string;
                     Xschn, Yschn: array of word; Zahl:word);
procedure WindateiFlinn (DateiZahl:byte; SDatei: string;
                         TitelS:array of string;
                         DLang, DMittel, DKurz: array of word; Zahl:word);
...
begin
  ...
  rewrite (Drucktext);
    ...
    begin
      ...
      Style := Style + [fsbold];
      writeln(Drucktext);
      writeln(Drucktext,SpaceS(15), Tx(1748));  {Wellman-Methode}
      writeln(Drucktext);
      writeln(Drucktext,SpaceS(40), Tx(1749));  {zweidimensionale Strain-Analyse}
      Size  := 10;
      Style := Style - [fsbold];
      writeln(DruckText);
      writeln(DruckText);
      ...
      writeln (Drucktext);
      writeln (Drucktext);
      writeln (Drucktext, ' ',Zahl,' ',Tx(1750));
      Writeln(Drucktext);
      for i := 1 to Zahl do               {Drucken der Daten}
      begin
        str(Xschn[i], XschS);
        str(Yschn[i], YschS);
        XschS  := RightS('0000'+XschS,4);
        YschS  := RightS('0000'+YschS,4);
        Write (Drucktext, '    ',XschS,' ',YschS);
        if i mod 10 = 0 then writeln(Drucktext);
      end;
    end;
  finally
...
begin
  ...
  rewrite (Drucktext);
    ...
    begin
      ...
      Style := Style + [fsbold];
      writeln(Drucktext);
      writeln(Drucktext,SpaceS(30), Tx(1534));  {Flinn-Diagramm}
      writeln(Drucktext);
      Size  := 10;
      Style := Style - [fsbold];
      writeln(DruckText);
      writeln(DruckText);
      writeln(Drucktext, SpaceS(40), Tx(190));         {Tag der Bearbeitung}
      writeln(Drucktext, SpaceS(40), WTagS,',  ', TagS,'.  ', MonS,'  ', JahrS,
      SpaceS(20), Tx(210), ZeitS);                     {Zeit:}
      writeln(DruckText);
      writeln(DruckText);
      for i := 0 to DateiZahl do
      ...
      writeln (Drucktext);
      writeln (Drucktext);
      writeln (Drucktext, SpaceS(40),Zahl,' ',Tx(101));
      Writeln(Drucktext);
      for i := 1 to Zahl do               {Drucken der Daten}
      begin
        LS := IntToStr(DLang[i]);
        MS := IntToStr(DMittel[i]);
        KS := IntToStr(DKurz[i]);
        LS := RightS('0000'+LS,4);
        MS := RightS('0000'+MS,4);
        KS := RightS('0000'+KS,4);
        Writeln (Drucktext, SpaceS(60),LS,',    ',MS,',    ',KS);
      end;
    end;
  finally
    CloseFile (Drucktext);
...
begin
  ...
  Assign (Datei,SDatei);
  rewrite (Datei);
  writeln(Datei,SpaceS(15), Tx(1748));  {Wellman-Methode}
  writeln(Datei);
  writeln(Datei,SpaceS(20), Tx(1749));  {zweidimensionale Strain-Analyse}
  writeln(Datei, Tx(190),' ', WTagS,', ', TagS,'. ', MonS,' ', JahrS, ' ', ZeitS);                     {Zeit:}
  for i := 0 to DateiZahl do
    writeln(Datei,Tx(256),'  ',TitelS[i]); {Dateiname}
  writeln (Datei, Zahl,'  ',Tx(1750));
  for i := 1 to Zahl do               {Drucken der Daten}
  begin
    str(Xschn[i], XschS);
    str(Yschn[i], YschS);
...
begin
  ...
  Assign (Datei,SDatei);
  rewrite (Datei);
  writeln(Datei,SpaceS(30), Tx(1534));  {Flinn-Diagramm}
  writeln(Datei, Tx(190),' ', WTagS,', ', TagS,'. ', MonS,' ', JahrS, ' ', ZeitS);                     {Zeit:}
  for i := 0 to DateiZahl do
    writeln(Datei,Tx(256),SpaceS(40),TitelS[i]); {Dateiname}
  ...
  begin
    ...
    MS := RightS('0000'+MS,4);
    KS := RightS('0000'+KS,4);
    Writeln (Datei, SpaceS(40),Ls,',   ',Ms,',   ',KS);
  end;
  CloseFile (Datei);
end;
...
begin
  ...
  7: begin
       UeberschriftS := SpaceS(10) + Tx(1407); {Linear aus Fl‰che und Pitchwinkel}
       SpaltenS      := Tx(1408);
     end;
  8: begin
       UeberschriftS := SpaceS(10) + Tx(1636);
       SpaltenS      := Tx(1637);
     end;
  12: begin
        UeberschriftS := SpaceS(10) + Tx(1631);
        SpaltenS      := Tx(1635);
      end;
  end;
  AssignPrn (Drucktext);
  ...
  try
    ...
    begin
      ...
      if EingabeS = 'Festplatte' then
        for i := 0 to DateiZahl do
          writeln(Drucktext, SpaceS(60),Tx(256),' ',TitelS[i]); {Dateiname}
      writeln(Drucktext);
      writeln(Drucktext,SpaceS(60),InttoStr(Anzahl),' ',Tx(101));
      writeln(DruckText);
      writeln(DruckText);
      ...
      begin
        ...
        1: begin
             ...
             Writeln(Drucktext, SpaceS(62), Az1S,'/',Fa1S,SpaceS(14), Az2S,'/',
                     Fa2S,SpaceS(14),AzErgS,'/',FaErgS);
           end;
        2: begin
             str(AzErg[i], AzErgS);
             AzErgS := RightS('000'+AzErgS,3);
             Writeln(Drucktext, SpaceS(61), Az1S,'/',Fa1S,SpaceS(14), Az2S,'/',
        ...
        5: begin
             ...
             AzErgS := RightS('000'+AzErgS,3);
             FaErgS := RightS('00'+FaErgS,2);
             Writeln(Drucktext, SpaceS(62), Az1S,'/',Fa1S,SpaceS(13), Az2S,'/',
        ...
        7: begin
             ...
             AzErgS := RightS('000'+AzErgS,3);
             FaErgS := RightS('00'+FaErgS,2);
             Writeln(Drucktext, SpaceS(60), Az1S,'/',Fa1S,SpaceS(17), Az2S,
                     SpaceS(15),AzErgS,'/',FaErgS);
           end;
       12: begin
             AzErgS := RightS('000'+IntToStr(AzErg[i]),3);
             FaErgS := RightS('00'+IntToStr(FaErg[i]),2);
             Writeln(Drucktext, SpaceS(59), Az1S,'/',Fa1S,SpaceS(12), Az2S,'/',
                     Fa2S,SpaceS(17),AzErgS,'/',FaErgS);
           end;
        end;
      end;
...
begin
  Uhr (JahrS, MonS, TagS, WtagS, ZeitS);
  case Fs of
    8: UeberschriftS := '  '+Tx(1475); {Fl‰chen:Azimut/Fallen in Streichen/Fallen Richtung}
    9: UeberschriftS := '  '+ Tx(1476); {Lineare:Azimut/Fallen in Streichen/Fallen Richtung}
  end;
...
begin
  ...
  begin
    ...
    1: begin
         ...
         AzErgS := RightS('000'+AzErgS,3);
         FaErgS := RightS('00'+FaErgS,2);
         Writeln(Datei, '     ',Az1S,'/',Fa1S,SpaceS(13), Az2S,'/',
                 Fa2S,SpaceS(14),AzErgS,'/',FaErgS);
       end;
    2: begin
         str(AzErg[i], AzErgS);
    ...
    7: begin
         ...
         FaErgS := RightS('00'+FaErgS,2);
         Writeln(Datei,'   ', Az1S,'/',Fa1S,SpaceS(13), Az2S,
                 SpaceS(12),AzErgS,'/',FaErgS);
       end;
    12: begin
          AzErgS := RightS('000'+IntToStr(AzErg[i]),3);
          FaErgS := RightS('00'+IntToStr(FaErg[i]),2);
          Writeln(Datei,' ', Az1S,'/',Fa1S,SpaceS(12), Az2S,'/',
                     Fa2S,SpaceS(12),AzErgS,'/',FaErgS);
        end;
    end;
  end;
  CloseFile (Datei);
...
begin
  if WeiterB[99] then Programm := 14;
  if (Programm = 12) or (Programm = 17)  or (Programm = 18)
  or (Programm = 29) then
  begin
  ...
  closefile (Datei);
  if (Programm = 12) or ((Programm = 17) and (MethodeS='Eigenwerte'))
  or (Programm = 18) or (Programm = 29) then RadiusS := IntToStr(Laenge div 2);
  ...
  if Programm = 17 then
  begin
    if MethodeS = 'Eigenwerte' then RadiusY := 9*RadiusX/21;  {Formfaktor}
    if MethodeS = 'Iteration' then
    begin
      if Druck = 1 then RadiusY := 0.71*RadiusX;
      if Druck = 2 then RadiusY := 0.612*RadiusX;
    end;  
    if MethodeS = 'Direkt' then
    begin
      if Druck = 1 then RadiusY := 0.52*RadiusX;
      if Druck = 2 then RadiusY := 0.52*RadiusX;
    end;
  end;
  if Programm = 18 then RadiusY := 3*RadiusX/2;   {Talpha}
  if (Programm = 29) and (DDruckS = 'Flinn') then RadiusY := RadiusX; {Flinn}
  if (Programm = 29) and (DDruckS = 'Hsu') then RadiusY := round(1.29*RadiusX);

  XKoordinate := Printer.PageWidth * StrToInt(XKoordS)/100
                  - RadiusX;
  YKoordinate := Printer.PageHeight * StrToInt(YKoordS)/100
                 - RadiusY;
  if (Programm = 17) and (MethodeS = 'Iteration') then
  begin
    Xkoordinate := StrToFloat(XKoordS)*Printer.PageWidth/100;
    Ykoordinate := StrToFloat(YKoordS)*Printer.PageHeight/100;
  end;
  if Programm = 27 then
  begin
    XKoordinate := Printer.PageWidth * StrToInt(XKoordS)/100;
