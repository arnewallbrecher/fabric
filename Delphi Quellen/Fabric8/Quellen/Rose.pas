...
procedure Verbindungslinie(Canvas: TCanvas; var X0, Y0: real; X, Y: real; Zm: TColor;
                           var Xmax, Ymax, Rmax: integer; Winkel:real);
...
begin
  ...
  begin
    Zm := DiagrFarbe;
    Verbindungslinie(Canvas, X0, Y0, X, Y, Zm, Xmax, Ymax, Rmax,B*Bo);
    Zm := RoseFarbe;
  end
  else
  begin
  ...
  if Plott = 0 then Verbindungslinie(Canvas, X0, Y0, Xi, Yi, Zm, Xmax, Ymax, Rmax, 0)
...
begin
  ...
  X2 := round(X);
  Y2 := round(Y);
  if (abs(X2) < 5) then X2 := round(5*sin(Winkel));
  if (abs(Y2) < 5) then Y2 := round(5*cos(Winkel));
  if (abs(X1) < 5) then X1 := round(5*sin(Winkel));
  if (abs(Y1) < 5) then Y1 := round(5*cos(Winkel));
  with Canvas do
  begin
    Pen.Color := Zm;
    if (abs(X1) >= 5) and (abs(Y1) >= 5) then
...
begin
  ...
  begin
    Anfangende(C,Cc,R,Xi,Yi,N);
    Zm := DiagrFarbe;
    Verbindungslinie(Canvas, X0, Y0, Xi, Yi, Zm, Xmax, Ymax, Rmax, 0);
    Zm := Rosefarbe;
  end
  else
  begin
  ...
  begin
      ...
      begin
        ...
        begin
          ...
          begin
            Y := R * cos (p*Bo) * N[i];
            X := R * sin (p*Bo) * N[i];
	    Verbindungslinie(Canvas, X0, Y0, X, Y, Zm, Xmax, Ymax, Rmax, p*Bo);
          end
	  else
          begin
	    Y := cos(p*Bo) * N[i] / Nm;
	...
	if Plott = 0 then
	  ...
	  Y := R * cos(j) * N[i];
          X := R * sin(j) * N[i];
	  Verbindungslinie(Canvas, X0, Y0, X, Y, Zm, Xmax, Ymax, Rmax, j);
        end
	else
        begin
	  Y := cos(j) * N[i] / Nm;
