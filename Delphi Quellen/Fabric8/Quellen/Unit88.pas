...
type
  TForm88 = class(TForm)
    ...
    PanelChiSpeichern: TPanel;
    ListBoxChiSpeichern: TListBox;
    ButtonChiAus: TButton;
    PanelRfSpeichern: TPanel;
    ...
    LabelText: TLabel;
    LabelReturn: TLabel;
    PanelPlazierenRf: TPanel;
    LabelPlazieren: TLabel;
    PanelTextCh: TPanel;
    Label2: TLabel;
    Label3: TLabel;
    MaskEditTextRf: TMaskEdit;
    MaskEditTextCh: TMaskEdit;
    PanelPlazierenCh: TPanel;
    Label4: TLabel;
    SpeedTextR: TSpeedButton;
    SpeedTextC: TSpeedButton;
    SpeedRueckR: TSpeedButton;
    SpeedRueckC: TSpeedButton;
    ...
    procedure ButtonChiAusClick(Sender: TObject);
    procedure ButtonRfAusClick(Sender: TObject);
    procedure TextRfPhiClick(Sender: TObject);
    procedure TextChiQClick(Sender: TObject);
    procedure RueckRfClick(Sender: TObject);
    procedure RueckChiClick(Sender: TObject);
    procedure ImageGrafikMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure ImageChiMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure MaskEditTextRfKeyPress(Sender: TObject; var Key: Char);
    procedure MaskEditTextChKeyPress(Sender: TObject; var Key: Char);
    procedure SpeedTextRClick(Sender: TObject);
    procedure SpeedTextCClick(Sender: TObject);
    procedure SpeedRueckRClick(Sender: TObject);
    procedure SpeedRueckCClick(Sender: TObject);
...
var
  ...
  BTextRf     : boolean;
  BtextCh     : boolean;
  TextBitmapRf: array[1..10] of TBitmap;
  TextBitmapCh: array[1..10] of TBitmap;
  textRfi     : byte;
  textChi     : byte;
  Druck       : byte;
...
begin
  ...
  ButtonChiAus.Caption     := Tx(1682);
  ButtonRfAus.Caption      := Tx(1682);
  ...
  SpeedTextR.Hint              := Hilf(739);
  SpeedTextC.Hint              := Hilf(740);
  SpeedRueckR.Hint             := Hilf(741);
  SpeedRueckC.Hint             := Hilf(742);
  LabelZahl.Caption        := '';
  LabelIndex.Caption       := '';
  ...
  PanelRfSpeichern.Visible := false;
  for i := 1 to 7 do
    ListBoxChiSpeichern.Items.Add (Tx(1611+i));
  PanelChiSpeichern.Visible := false;
  Wieder := false;
  PanelTextRf.Visible := false;
  PanelTextCh.Visible := false;
  BTextRf             := false;
  BtextCh             := false;
  textRfi             := 0;
  textChi             := 0;
  RueckRf.Enabled     := false;
  RueckChi.Enabled    := false;
end;
...
procedure TForm88.ButtonRfAusClick(Sender: TObject);
begin
  PanelRfSpeichern.Visible := false;
end;
...
begin
  PanelChiSpeichern.Visible := true;
end;
...
procedure TForm88.ButtonChiAusClick(Sender: TObject);
begin
  PanelChiSpeichern.Visible := false;
end;
...
procedure TForm88.MaskEditLaengeChange(Sender: TObject);
var
  Datei : TextFile;
  i     : word;
begin
...
procedure TForm88.MaskEditLinksChange(Sender: TObject);
var
  Datei : TextFile;
  i     : word;
begin
...
procedure TForm88.MaskEditObenChange(Sender: TObject);
var
  Datei : TextFile;
  i     : word;
begin
...
procedure TForm88.RadioLinksClick(Sender: TObject);
begin
  BuendigS := 'Links';
  Gedrueckt[3] := true;
  if (Gedrueckt[1] = true) and (Gedrueckt[2] = true)
...
procedure TForm88.RadioZentriertClick(Sender: TObject);
begin
  BuendigS := 'Zentriert';
  Gedrueckt[3] := true;
  if (Gedrueckt[1] = true) and (Gedrueckt[2] = true)
...
procedure TForm88.RadioRechtsClick(Sender: TObject);
begin
  BuendigS := 'Rechts';
  Gedrueckt[3] := true;
  if (Gedrueckt[1] = true) and (Gedrueckt[2] = true)
...
begin
  ...
  if Druck = 1 then
    Hoehe   := round(PLaenge*ImageGrafik.Height/ImageGrafik.Width);
  if Druck = 2 then
    Hoehe   := round(PLaenge*ImageChi.Height/ImageChi.Width);
  with Fenster do
  begin
    Left   := LRand;
    Top    := ORand;
  ...
  try
    if Druck = 1 then
      Bitmap.Assign(ImageGrafik.picture.graphic);
    if Druck = 2 then
      Bitmap.Assign(ImageChi.picture.graphic);    
    with Printer do
    begin
...
procedure TForm88.TextRfPhiClick(Sender: TObject);
begin
  PanelTextRf.Visible := true;
  MaskEditTextRf.SetFocus;
  BTextRf := true;
  MaskEditTextRf.Clear;
  PanelPlazierenRf.Visible := false;
end;

procedure TForm88.SpeedTextRClick(Sender: TObject);
begin
  TextRfPhiClick(Sender);
end;

procedure TForm88.TextChiQClick(Sender: TObject);
begin
  ImageChi.Enabled := true;
  PanelTextCh.Visible := true;
  MaskEditTextCh.SetFocus;
  BTextCh := true;
  MaskEditTextCh.Clear;
  PanelPlazierenCh.Visible := false;
end;

procedure TForm88.SpeedTextCClick(Sender: TObject);
begin
  TextChiQClick(Sender);
end;

procedure TForm88.MaskEditTextRfKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = chr(13) then
  begin
    TeRfS := MaskEditTextRf.Text;
    PanelPlazierenRf.Visible := true;
  end;
end;

procedure TForm88.MaskEditTextChKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = chr(13) then
  begin
    TeChS := MaskEditTextCh.Text;
    PanelPlazierenCh.Visible := true;
  end;
end;

procedure TForm88.ImageGrafikMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if BTextRf then
  begin
    inc (textRfi);
    TextBitmapRf[textRfi] := Tbitmap.Create;
    TextBitmapRf[textRfi].Assign(ImageGrafik.Picture);
    with ImageGrafik.Canvas do
    begin
    ...
    ImageGrafik.Canvas.TextOut(x,y,TeRfS);
    PanelTextRf.Visible := false;
    BTextRf := false;
    RueckRf.Enabled     := true;
    SpeedRueckR.Enabled := true
  end;
end;

procedure TForm88.ImageChiMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if BTextCh then
  begin
    inc (textChi);
    TextBitmapCh[textChi] := Tbitmap.Create;
    TextBitmapCh[textChi].Assign(ImageChi.Picture);
    with ImageChi.Canvas do
    begin
      Brush.Color := DiagrFarbe;
      Font.Name   := DSchrift;
      Font.Size   := DGroesse;
    ...
    ImageChi.Canvas.TextOut(x,y,TeChS);
    PanelTextCh.Visible := false;
    BTextCh             := false;
    RueckChi.Enabled    := true;
    SpeedRueckC.Enabled := true;
  end;
end;

procedure TForm88.RueckRfClick(Sender: TObject);
begin
  ImageGrafik.Canvas.Draw (0,0,TextBitmapRf[textRfi]);
  if textRfi >= 1 then
  begin
    TextBitmapRf[textRfi].free;
    Dec(textRfi);
  end;
  if textRFi = 0 then
  begin
    RueckRf.Enabled := false;
    SpeedRueckR.Enabled := false;
  end;
end;

procedure TForm88.SpeedRueckRClick(Sender: TObject);
begin
  RueckRfClick(Sender);
end;

procedure TForm88.RueckChiClick(Sender: TObject);
begin
  ImageChi.Canvas.Draw (0,0,TextBitmapCh[textChi]);
  if textChi >= 1 then
  begin
    TextBitmapCh[textChi].free;
    Dec(textChi);
  end;
  if textChi = 0 then
  begin
    RueckChi.Enabled    := false;
    SpeedRueckC.Enabled := false;
  end;
end;

procedure TForm88.SpeedRueckCClick(Sender: TObject);
begin
  RueckChiClick(Sender);
end;
