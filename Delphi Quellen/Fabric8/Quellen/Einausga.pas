...
uses Globvar, Dialogs, Basic, Texte, Standard, SysUtils,
     Dateien, Controls, FileCtrl, OleServer, Shellapi;
...
procedure PunkteSpeichern (DateiS:string; N:word; Xkoord,  //Wellman
                     Ykoord: array of word);
...
procedure SchreibKreis (DateiS: string;N: word;Azimut, Fallen: array of word;
                        Kreis: array of word);
procedure AchsenSpeichern (DateiS: string; N: word;
                           LangA, MittelA, KurzA: array of word);
...
procedure PunkteSpeichern (DateiS:string; N:word; Xkoord,
                     Ykoord: array of word);
var
  i          :integer;
  Datei      :text;
begin
  ...
  writeln(Datei, DateiS);
  if n >= 1 then
    for i := 1 tO n do Writeln(Datei, Xkoord[i], ',', Ykoord[i]);
  Beep;
  CloseFile (Datei);
end;
...
begin
  ...
  if Programm = 5 then ExtensionS := 'SCI';
...
begin
  ...
  writeln(Datei, DateiS);
  if n >= 1 then
    for i := 1 to n do Writeln(Datei, LangA[i], ',', MittelA[i],',',KurzA[i]);
  Beep;
  CloseFile (Datei);
end;
end.
