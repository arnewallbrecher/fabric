...
uses
  Texte, Forms, Classes, Controls, StdCtrls, ExtCtrls,
  Graphics, Globvar, Dialogs, Plotten, Windows, SysUtils,
  Math, Menus, Clipbrd, Messages, Mask, Drucken, Printers, Buttons,
  ComCtrls, Shellapi, Standard, GifImage, Bmp2Tiff, Jpeg, Basic,
  ExtDlgs, ehsHelpRouter;
type
  TForm75 = class(TForm)
    ButtonStart  : TButton;
...
begin
  ...
  begin
    ...
    Brush.Style := bsSolid;
    FloodFill (5,5,UmfangFarbe,fsBorder);
  end;
end;
...
procedure TForm75.speichernBMPClick(Sender: TObject);
var
  Rahmen : TRect;
begin
  Bitmap        := TBitmap.Create;
...
procedure TForm75.SpeedBMPClick(Sender: TObject);
begin
  speichernBMPClick(Sender);
end;

procedure TForm75.speichernGIFClick(Sender: TObject);
var
  Bitmap  : TBitmap;
  GifBild : TGifImage;
  Rahmen  : TRect;
...
procedure TForm75.SpeedGIFClick(Sender: TObject);
begin
  speichernGIFClick(Sender);
end;

procedure TForm75.speichernTIFClick(Sender: TObject);
var
  Rahmen : TRect;
begin
  Bitmap        := TBitmap.Create;
...
procedure TForm75.SpeedTIFClick(Sender: TObject);
begin
  speichernTIFClick(Sender);
end;

procedure TForm75.speichernJPGClick(Sender: TObject);
var
  DateiJ   : string;
  Rahmen   : TRect;
  Bild     : TBitmap;
...
procedure TForm75.SpeedJPGClick(Sender: TObject);
begin
  speichernJPGClick(Sender);
end;

procedure TForm75.speichernWMFClick(Sender: TObject);
var
  Metafile   : TMetafile;
...
procedure TForm75.SpeedWMFClick(Sender: TObject);
begin
  speichernWMFClick(Sender);
end;

procedure TForm75.speichernEMFClick(Sender: TObject);
var
  Metafile   : TMetafile;
  MetaCanvas : TMetafileCanvas;
  Bitmap     : TBitmap;
...
procedure TForm75.SpeedEMFClick(Sender: TObject);
begin
  speichernEMFClick(Sender);
end;

procedure TForm75.speichernUBMPClick(Sender: TObject);
begin
...
procedure TForm75.speichernUGIFClick(Sender: TObject);
begin
  SavePictureDialog1.DefaultExt := GraphicExtension(TGIFImage);
...
procedure TForm75.speichernUTIFClick(Sender: TObject);
begin
  SavePictureDialog1.DefaultExt := 'TIF';
  SavePictureDialog1.Filter     := 'TIF';
  if SavePictureDialog1.Execute then
  begin
    CurrentFile := SavePictureDialog1.Filename;
    SpeichernTIFClick(Sender);
...
procedure TForm75.speichernUJPGClick(Sender: TObject);
begin
...
procedure TForm75.speichernUWMFClick(Sender: TObject);
begin
  SavePictureDialog1.DefaultExt := GraphicExtension(TMetaFile);
...
procedure TForm75.speichernUEMFClick(Sender: TObject);
begin
  SavePictureDialog1.DefaultExt := GraphicExtension(TMetaFile);
  SavePictureDialog1.Filter     := GraphicFilter(TMetaFile);
  if SavePictureDialog1.Execute then
...
begin
  ...
  begin
    ...
    HelpRouter1.HelpJump(DateiS,'html\Deutsch\zaeh022i.htm');
  end;
  if SpracheS = 'ENGLISH' then
  begin
    DateiS := DirectS+'\'+'Fabric-Help-Html';
    HelpRouter1.HelpJump(DateiS,'html\English\Counting-calotte.htm');
  end;
  HelpRouter1.free;
end;
