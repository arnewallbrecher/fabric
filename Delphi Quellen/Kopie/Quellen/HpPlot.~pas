...
begin
  ...
  AltYmitte := Ymitte;
  if GeraetS = 'Plotter' then
    DiagNameS := InputBox(Tx(439),Tx(887),' '); {Diagramm-Beschriftung eingeben}
  XpMitte := 0;
  YpMitte := 0;
  H       := 0;
  ...
  if GeraetS = 'Plotter' then
  begin
    X := round(Xkoord*100*Faktor*DFak);
    Y := round(Ykoord*100*Faktor*DFak);
    R := round(HpRadius*100*Faktor*DFak);
    with Canvas do
    begin
      Pen.Color   := Farbe;
      Brush.Style := bsClear;
...
begin
  ...
  if GeraetS = 'Plotter' then
  begin
    X0 := round(Xkoord*100*Faktor*DFak);
    Y0 := round((Ykoord)*100*Faktor*DFak);
    with Canvas do
    begin
      ...
      moveto (X0, round(Y0-HpRadius*100*Faktor*DFak));
      lineto (X0, round(Y0-HpRadius*100*Faktor*DFak-1.5*Hoehe));
      lineto (X0-Breite,round(Y0-HpRadius*100*Faktor*DFak-Hoehe));
      moveto (X0, round(Y0-HpRadius*100*Faktor*DFak-1.5*Hoehe));
      lineto (X0+Breite,round(Y0-HpRadius*100*Faktor*DFak-Hoehe));
      Textout (X0-Breite, round(Y0-HpRadius*100*Faktor*DFak-Hoehe), NordS);
      TextS := ZahlS + ' ' + TtS;
      Font.Size := round(0.5*Nord);
      if ZahlGlob <> 0 then
        Textout (round(X0-HpRadius*100*Faktor*DFak), round(Y0-HpRadius*100*Faktor*DFak-10), TextS);
      Breite := TextWidth (AaS) div 2;
      Textout (X0-Breite, round(Y0-HpRadius*100*Faktor*DFak+2*HpRadius*100*Faktor*DFak
               +HpRadius*5), AaS);
...
begin
  ...
  begin
    ...
    if GeraetS = 'Plotter' then
    begin
      Xpunkt  := round (100*Faktor*DFak*(Xkoord+X*HpRadius));
      Ypunkt  := round (100*Faktor*DFak*(YKoord-Y*HpRadius));
      Radius1 := round (100*Faktor*DFak*HpRadius / 100);
    end;
    if Programm = 21 then
    begin
      if Talbot[i] then
...
begin
  ...
  if GeraetS = 'Plotter' then
  begin
    XPMitte := round(XKoord*100*Faktor*DFak);
    YPMitte := round(YKoord*100*Faktor*DFak);
    Rint    := round(HpRadius*100*Faktor*DFak);
...
begin
  ...
  if GeraetS ='Plotter' then
  begin
    XMitte := round(XKoord*100*Faktor*DFak);
    YMitte := round(YKoord*100*Faktor*DFak);
    Rint   := round(HpRadius*100*Faktor*DFak);
  end;
  Dist := 100;
...
begin
  ...
  begin
    ...
    begin
      Pen.Color := Farbe;
      Xa := round(Xanfang*100*Faktor*DFak);
      Ya := round((Yanfang*DFak+Laenge*DFak)*100*Faktor);
      for i := 1 to Teile - Minus do
      begin
        XTick  := round(100*Faktor*DFak*(Xanfang+i*Abstand));
...
begin
  ...
  begin
    ...
    if GeraetS = 'Plotter' then
    begin
      TickY := round (Faktor*(Xanfang*100*DFak+50*Tick));
      Xanf  := round(100*Faktor*XAnfang*DFak);
      with Printer.Canvas do
      begin
        Pen.Color := Farbe;
        for i := 1 to Teile - Minus do
        begin
          YTick := round(Faktor*100*DFak*(Yanfang+i*Abstand));
          moveto (Xanf,YTick);
          lineto (TickY,YTick);
        end;
      end;
  ...
  begin
    ...
    begin
      ...
      Xanw  := round(100*DFak*Faktor*Xanfang);
      Xenw  := round(100*Faktor*DFak*(Xanfang + Tick/2));
      Canvas.Pen.Color := Farbe;
      for i := 1 to Teile - 1 do
      begin
        Yanw := round(100*Faktor*DFak*(Yanfang+i*Abstand));
        Yenw := Yanw;
        Yan := Yanfang + i*Abstand;
        Yende := Yan;
...
begin
  ...
  begin
    ...
    if GeraetS = 'Plotter' then
    begin
      if Programm = 10 then YAbstand := DFak*Laenge/40;
      if Programm = 13 then YAbstand := DFak*Laenge/60;
      if Programm = 18 then YAbstand := DFak*Laenge/15;
      YZahlW := round(100*Faktor*DFak*(YNull+YAbstand));
      Canvas.Font.Color := Farbe;
//      Canvas.Font.Size  := round(Laenge);
      for i := 1 to Teile - Minus do
      begin
        Zahl    := round(i*FaktorX);
        Distanz := round(100*Faktor*DFak*(XNull+i*Einheit));
        Zahls := IntToStr (Zahl);
        Printer.Canvas.Textout (Distanz, YZahlW,ZahlS);
      end; {for}
    end; {if GeraetS = Plotter}
  end;
  if Programm = 10 then
  begin
    ...
    if GeraetS = 'Plotter' then
    begin
      XPlotter := round(100*Faktor*DFak*(XNull+Laenge));
      YPlotter := round(100*Faktor*DFak*(YNull+Laenge/20));
      Canvas.Font.Size := round(Laenge);
  ...
  begin
    ...
    begin
      Abstand := Laenge / 10;
      XW := round(100*Faktor*DFak*(XNull+Laenge+Abstand));
  ...
  begin
    ...
    begin
      ...
      SFaktor := Ende / Teile;
      RgS     := 'a';
      YZahlw  := round(100*Faktor*DFak*(YNull+2*Laenge+0.05*Laenge));
      for i := 1 to Teile - Minus do
      begin
        str (round(i*SFaktor),ZahlS);
        Distanzw := round(100*Faktor*DFak*(XNull+i*Einheit));
        Canvas.Textout (Distanzw, Yzahlw, ZahlS);
      end;
        Xw := round(100*Faktor*DFak*(XNull+Laenge));
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        XZahl   := round(100*Faktor*DFak*(XNull-Laenge/20));
        Distanz := round(100*Faktor*DFak*(YNull+Laenge-i*Einheit-Add)-50);
    ...
    begin
      ...
      if GeraetS = 'Plotter' then
      begin
        Yy := round(100*Faktor*YNull*DFak);
    ...
    begin
      ...
      if GeraetS = 'Plotter' then
      begin
        XW := round((XNull-0.5*Abstand)*100*Faktor*DFak);
        YW := round((YNull-0.8*Abstand)*100*Faktor*DFak);
  ...
  begin
    ...
    begin
      H := Laenge/5;
      Xzahlw := round(100*Faktor*DFak*(Xnull-Laenge/15));
      X := Xnull - Laenge/20;
      for i := 1 to Teile do
      begin
        str ((i/10):3:1,ZahlS);
        Yzahlw := round(100*Faktor*DFak*(Ynull + Laenge - i*Einheit));
        Y := Ynull + i*Einheit;
        Canvas.Textout (Xzahlw, Yzahlw, ZahlS);
      end;
      RgS := 'T/T0';
      Yw := round (100*Faktor*DFak*(Ynull)-100);
      Xzahlw := round(100*Faktor*DFak*(Xnull - Laenge/15));
      X := Xnull - Laenge/12;
      Y := Ynull + Laenge + 0.5;
...
begin
  ...
  begin
    ...
    begin
      Pen.Color := Farbe;
      Xan := round(XNull*100*Faktor*DFak);
      Xen := round((XNull+Laenge*cos(LWinkel*Bo))
                   *100*Faktor*DFak);
      Yan := round(YNull*100*Faktor*DFak);
...
begin
  ...
  if GeraetS = 'Plotter' then
  begin
    Xpunkt  := round(100 * Faktor * DFak*(Xanfang+Xsymbol));
    Ypunkt  := round(100 * Faktor * DFak*(Yanfang+Laenge-Ysymbol));
    if Programm = 16 then
      Ypunkt := round(DFak*100*Faktor*(YAnfang+Laenge*sin(60*Bo)
                -YSymbol));
    Radius  := round(DFak*100 * Faktor * Laenge / 100);
  end;
  if GeraetS <> 'Plotter' then
  begin
    case Nsymbol of
...
begin
  ...
  begin
    ...
    X1  := round (X*100*Faktor*DFak);
    Y1  := round (100*Faktor*Y*DFak);
    X2  := round (X1 + 100*Faktor*DFak*Laenge);
    Y2  := Y1;
    X3  := round (X1 + 50*Faktor*DFak*Laenge);
    Y3  := round (100*Faktor*DFak*(Y+Laenge*Tan(60*Bo)/2));
    L   := Laenge*3/4;
    Xp  := X1;
    Wd  := Laenge/50;
    Hd  := Laenge/35;
...
//    Canvas.Textout (Xpw, Yew, 'E3-E2');
    Canvas.Textout (Xgw, Ypw, Tx(836)+' '+'2(E2-E1)');    {Gürtel}
//    Canvas.Textout (Xgw, Yew, '2(E2-E1)');
    Canvas.Textout (Xzw, Yzw, Tx(837)+' '+'3E1');    {Zufall}
//    Canvas.Textout (Xzw, Yzew, '3E1');
    Hoehe := round (5*L*Tan(60*Bo)/2);
    Y1w  := Y1;
    Yt1w := Y1+Hoehe;
    for i := 1 to 9 do
    begin
      Xt1w := round(X1+10*Faktor*DFak*i*Laenge);
      Xt2w := round(Xt1w-5*Faktor*DFak*L*Tan(30*Bo));
      Xt3w := round(Xt1w+5*Faktor*DFak*L*Tan(30*Bo));
    ...
    for i := 1 to 9 do
    begin
      Xt1w := round (X1+5*Faktor*DFak*i*Laenge);
    ...
    for i := 1 to 9 do
    begin
      Xt1w := round (X2 - 5*Faktor*DFak*i*Laenge);
      Xt2w := round (Xt1w - Hoehe/cos(30*Bo));
...
begin
  ...
  begin
    ...
    Ytra4d := Ytra3d;
    Xtra1w  := round (Xtra1d*100*Faktor*DFak);
    Xtra2w  := round (Xtra2d*100*Faktor*DFak);
    Xtra3w  := round (Xtra3d*100*Faktor*DFak);
    Xtra4w  := round (Xtra4d*100*Faktor*DFak);
    Ytra1w  := round (Ytra1d*100*Faktor*DFak);
    Ytra2w  := round (Ytra2d*100*Faktor*DFak);;
    Ytra3w  := round (Ytra3d*100*Faktor*DFak);
    Ytra4w  := round (Ytra4d*100*Faktor*DFak);
    Farben (Canvas, KoordinFarbe);
    {Plotten des Trapezes:}
    with Canvas do
    begin
    ...
    {Skalierung:}
      ...
      begin
        ...
        X1w := round(X1d*100*Faktor*DFak);
        X2w := round(X2d*100*Faktor*DFak);
        Y1w := round(Y1d*100*Faktor*DFak);
        Y2w := round(Y2d*100*Faktor*DFak);
        Moveto (X1w, Y1w);
        Lineto (X2w, Y2w);
        X2d := X1d - cos(60*Bo)*Seite/50;
        X2w := round(X2d*100*Faktor*DFak);
      ...
      Font.Size  := 12;
      Brush.Style := bsClear;
      Ylambdaw := round(Ytra1d*100*Faktor*DFak+100*Faktor*DFak*Seite/50);
      Xlambdaw := round(Xtra2d*100*Faktor*DFak-50*Faktor*DFak*Seite/35);
    ...
    begin
      ...
      Ylambdad := Ytra1d - sin(60*Bo)*Kante*Lambda1[i];
      Xw := round(Xlambdad*100*Faktor*DFak);
      Yw := round(Ylambdad*100*Faktor*DFak);
      Rw := round(Seite);
      Canvas.Ellipse (Xw-Rw, Yw-Rw, Xw+Rw, Yw+Rw);
      Xlambdaw := round(Xlambdad*100*Faktor*DFak)-Rw;
      YLambdaw := round(Ylambdad*100*Faktor*DFak+Seite*3/2);
      ...
      begin
        ...
        case m of
          0: begin
               ...
               Xpa := Xm+(Punkt1[j,1].x-Xmitte)*Fak*DFak;
               Ypa := Ym-(YMitte-Punkt1[j,1].y)*Fak*DFak;
             end;
          1: begin
               ...
               XSA := IntToStr(round(Xa));
               YSA := IntToStr(round(Ya));
               Xpa := Xm+(Punkt2[j,1].x-Xmitte)*Fak*DFak;
               Ypa := Ym-(YMitte-Punkt2[j,1].y)*Fak*DFak;
             end;
          2: begin
               ...
               XSA := IntToStr(round(Xa));
               YSA := IntToStr(round(Ya));
               Xpa := Xm+(Punkt3[j,1].x-Xmitte)*Fak*DFak;
               Ypa := Ym-(YMitte-Punkt3[j,1].y)*Fak*DFak;
             end;
          3: begin
               ...
               XSA := IntToStr(round(Xa));
               YSA := IntToStr(round(Ya));
               Xpa := Xm+(Punkt4[j,1].x-Xmitte)*Fak*DFak;
               Ypa := Ym-(YMitte-Punkt4[j,1].y)*Fak*DFak;
             end;
          4: begin
               ...
               XSA := IntToStr(round(Xa));
               YSA := IntToStr(round(Ya));
               Xpa := Xm+(Punkt5[j,1].x-Xmitte)*Fak*DFak;
               Ypa := Ym-(YMitte-Punkt5[j,1].y)*Fak*DFak;
             end;
        end;
        if GeraetS = 'HPGL' then
        begin
...
//          Plotte ('Pd;Pa'+XSA+','+YSA+';');
        ...
        begin
          ...
          begin
            ...
            begin
              Canvas.Pen.Width := StrichI[m];
              Canvas.lineto (round(Xm+xmittel*Fak*DFak),
                             round(Ym-Ymittel*Fak*DFak));
            end;
          end; // mod
        end; // i
        if GeraetS = 'DXF' then
      ...
      begin
        ...
        case m of
          0: begin
               ...
               XSA := IntToStr(round(Xa));
               YSA := IntToStr(round(Ya));
               Xpa := Xm+(abs(Punkt1[j,1].x)-Xmitte)*Fak*DFak;
               Ypa := Ym-(YMitte-Punkt1[j,1].y)*Fak*DFak;
          ...
          1: begin
               ...
               XSA := IntToStr(round(Xa));
               YSA := IntToStr(round(Ya));
               Xpa := Xm+(abs(Punkt2[j,1].x)-Xmitte)*Fak*DFak;
               Ypa := Ym-(YMitte-Punkt2[j,1].y)*Fak*DFak;
          ...
          2: begin
               ...
               XSA := IntToStr(round(Xa));
               YSA := IntToStr(round(Ya));
               Xpa := Xm+(abs(Punkt3[j,1].x)-Xmitte)*Fak*DFak;
               Ypa := Ym-(YMitte-Punkt3[j,1].y)*Fak*DFak;
          ...
          3: begin
               ...
               XSA := IntToStr(round(Xa));
               YSA := IntToStr(round(Ya));
               Xpa := Xm+(abs(Punkt4[j,1].x)-Xmitte)*Fak*DFak;
               Ypa := Ym-(YMitte-Punkt4[j,1].y)*Fak*DFak;
          ...
          4: begin
               ...
               XSA := IntToStr(round(Xa));
               YSA := IntToStr(round(Ya));
               Xpa := Xm+(abs(Punkt5[j,1].x)-Xmitte)*Fak*DFak;
               Ypa := Ym-(YMitte-Punkt5[j,1].y)*Fak*DFak;
