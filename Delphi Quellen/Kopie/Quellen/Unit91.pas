...
uses
  ...
  StdCtrls, ExtCtrls, Mask, Standard, Texte, Globvar, Menus, Basic, Plotten,
  ClipBrd, GifImage, Bmp2Tiff, ReadTiff, Jpeg, ExtDlgs, Printers, Drucken,
  ComCtrls, ShellApi, Buttons, ehsHelpRouter;
...
type
  TForm91 = class(TForm)
    ...
    procedure TIFClick(Sender: TObject);
    procedure JPGClick(Sender: TObject);
    procedure WMFClick(Sender: TObject);
    procedure EMFClick(Sender: TObject);
    procedure UBMPClick(Sender: TObject);
    procedure UGifClick(Sender: TObject);
    procedure UTifClick(Sender: TObject);
    procedure UJPGClick(Sender: TObject);
    procedure UWmfClick(Sender: TObject);
    procedure UEmfClick(Sender: TObject);
    ...
    procedure SpeedBEMFClick(Sender: TObject);
...
begin
  ...
  SpeedBMP.Visible         := false;
  SpeedGIF.Visible         := false;
  SpeedTif.Visible         := false;
  SpeedJPG.Visible         := false;
  SpeedWMF.Visible         := false;
  SpeedBEMF.Visible        := false;
...
begin
  ...
  SpeedBMP.Visible         := true;
  SpeedGIF.Visible         := true;
  SpeedTif.Visible         := true;
  SpeedJPG.Visible         := true;
  SpeedWMF.Visible         := true;
  SpeedBEMF.Visible        := true;
...
procedure TForm91.BMPClick(Sender: TObject);
var
  Rahmen : TRect;
  Bitmap : TBitmap;
begin
  Bitmap        := TBitmap.Create;
  Bitmap.Width  := ImageMohr.Width;
  Bitmap.Height := ImageMohr.Height;
  Rahmen        := Rect(0,0,Bitmap.Width,Bitmap.Height);
  Bitmap.Canvas.Stretchdraw(Rahmen,ImageMohr.Picture.Graphic);
  try
    if (CurrentFile <> '') and (UCaseS(RightS(CurrentFile,3)) = 'BMP')
    then Bitmap.SaveToFile(CurrentFile)
    else UBMPClick(Sender);
  finally
    Bitmap.Free;
    Bitmap   := nil;
  end;
...
procedure TForm91.SpeedBMPClick(Sender: TObject);
begin
  BMPClick(Sender);
end;


procedure TForm91.GIFClick(Sender: TObject);
var
  Bitmap  : TBitmap;
  GifBild : TGifImage;
  Rahmen  : TRect;
...
procedure TForm91.SpeedGIFClick(Sender: TObject);
begin
  GIFClick(Sender);
end;

procedure TForm91.TIFClick(Sender: TObject);
var
  Rahmen : TRect;
  Bitmap : TBitmap;
begin
...
procedure TForm91.SpeedTIFClick(Sender: TObject);
begin
  TIFClick(Sender);
end;

procedure TForm91.JPGClick(Sender: TObject);
var
  DateiJ   : string;
  Rahmen   : TRect;
  Bild     : TBitmap;
...
begin
  ...
  Bild        := TBitmap.Create;
  Bild.Width  := ImageMohr.Width;
  Bild.Height := ImageMohr.Height;
  Rahmen      := Rect(0,0,Bild.Width,Bild.Height);
  Bild.Canvas.Stretchdraw(Rahmen,ImageMohr.Picture.Graphic);
  try
    with JpgDatei do
    begin
      Assign(Bild);
...
procedure TForm91.SpeedJPGClick(Sender: TObject);
begin
  JpgClick(Sender);
end;

procedure TForm91.WMFClick(Sender: TObject);
var
  Metafile   : TMetafile;
...
begin
  ...
  try
    ...
    Bitmap.Canvas.Stretchdraw(Rahmen,ImageMohr.Picture.Graphic);
    try
      Bitmap.Assign(ImageMohr.Picture);
    ...
    else UWMFClick(Sender);
  finally
    Metafile.Free;
  end;
end;

procedure TForm91.SpeedWMFClick(Sender: TObject);
begin
  WMFClick(Sender);
end;

procedure TForm91.EMFClick(Sender: TObject);
var
  Metafile   : TMetafile;
  MetaCanvas : TMetafileCanvas;
  Bitmap     : TBitmap;
begin
  ...
  try
    ...
    try
      Bitmap.Assign(ImageMohr.Picture);
      Metafile.Height := ImageMohr.Width;
      Metafile.Width  := ImageMohr.Height;
      MetaCanvas := TMetafileCanvas.Create(Metafile, 0);
      try
        MetaCanvas.Draw(0, 0, Bitmap);
      finally
    ...
    then MetaFile.SaveToFile(CurrentFile)
    else UEMFClick(Sender);
  finally
    Metafile.Free;
  end;
end;

procedure TForm91.SpeedBEMFClick(Sender: TObject);
begin
  EMFClick(Sender);
end;

procedure TForm91.UBMPClick(Sender: TObject);
begin
...
procedure TForm91.UGifClick(Sender: TObject);
begin
...
procedure TForm91.UTifClick(Sender: TObject);
begin
...
procedure TForm91.UJPGClick(Sender: TObject);
begin
...
procedure TForm91.UWmfClick(Sender: TObject);
begin
...
procedure TForm91.UEmfClick(Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    HelpRouter1.HelpJump(DateiS,'html\Deutsch\mohr-Eingabe.htm');
  end;
  if SpracheS = 'ENGLISH' then
  begin
    DateiS := DirectS+'\'+'Fabric-Help-Html';
    HelpRouter1.HelpJump(DateiS,'html\English\Mohr-Input.htm');
  end;
  HelpRouter1.free;
end;
