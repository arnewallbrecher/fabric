...
begin
  ...
  if Berechnung >7 then setlength (SpZeile, MaxDaten);
  WeiterB[4] := false;
...
begin
  ...
  FalErgFeldP := Nil;
  if Berechnung >7 then SpZeile := NIL;
  if not WeiterF then
  begin
    AziFeld := Nil;
    FalFeld := NiL;
...
begin
  Erweiterung (ExtenS);
  if Berechnung = 10 then ExtenS := 'PLB';
  if Berechnung = 11 then ExtenS := 'LIB';
  ...
  begin
    ...
    if Berechnung > 9 then SpeichernS(CurrentFile,SpZeile,Anzahl);
  end;
  CurrentFile := '';
end;
...
begin
  ...
  or (Berechnung = 11) then ExtenS := 'LIN';
  if (Berechnung = 2) or (Berechnung = 3) or (Berechnung = 6)
  then ExtenS := 'ANG';
  if (Berechnung = 4) or (Berechnung = 10) then ExtenS := 'PLA';
  if Berechnung = 8 then ExtenS := 'PLB';
  if Berechnung = 9 then ExtenS := 'LIB';
  SaveDialog1.DefaultExt := ExtenS;
  SaveDialog1.Filter     := ExtenS;
  if SaveDialog1.Execute then
  begin
    ...
    or (Berechnung = 7) then Speichern2 (CurrentFile, Anzahl,
...
var
  MaxStrei   : word;
begin
  if Winkel = 1 then MaxStrei := 180 else MaxStrei := 200;
  if Key = chr(13) then
  begin
...
begin
  ...
  begin
    ...
    HelpRouter1.HelpJump(DateiS,'html\Deutsch\bere0rcc.htm');
  end;
  if SpracheS = 'ENGLISH' then
  begin
    DateiS := DirectS+'\'+'Fabric-Help-Html';
    HelpRouter1.HelpJump(DateiS,'html\English\calc0rcc.htm');
  end;
  HelpRouter1.free;
end;
